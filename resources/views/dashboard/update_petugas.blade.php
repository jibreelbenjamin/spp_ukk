<x-app :halaman='$title' :page='$page'>
  <form action="{{ route('petugas.update', ['user' => $user->id_user]) }}" method="post"\>
    <div>
      @csrf
      @method('PUT')
      ...
      <div class="mb-5 first:pt-0 last:pb-0">
        ...
        <select name="role" data-hs-select='{
          ...
          "toggleClasses": "mt-2 space-y-3 hs-select-disabled:pointer-events-none hs-select-disabled:opacity-50 relative py-2.5 px-4 pe-9 flex items-center text-nowrap w-full cursor-pointer bg-white border border-gray-200 rounded-lg text-start text-sm focus:border-blue-500 focus:ring-blue-500 before:absolute before:inset-0 before:z-1",
          ...
          <option value="admin" {{ $user->role === 'admin' ? 'selected' : '' }} data-hs-select-option='{
          ...
          <option value="petugas" {{ $user->role === 'petugas' ? 'selected' : '' }} data-hs-select-option='{
